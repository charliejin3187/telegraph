...
def reverse(bot: Bot, update: Update, args: List[str]):
    ...
    if reply:
        ...
        if len(splatargs) == 3:                
            ...
            msg.reply_text("/lens <link> <amount of images to return.>")
            return
        try:
        ...
        msg.reply_markdown("Please reply to a sticker, or an image to search it!\nDo you know that you can search an image with a link too? `/lens [picturelink] <amount>`.")
        return
    ...
    try:
        ...
        if response != 400:
            xx = bot.send_message(chat_id, "Image was successfully uploaded to Google."
                                  "\nSearching For the most relevant Results.", reply_to_message_id=rtmid)
        else:
